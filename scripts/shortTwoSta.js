...
$('#buttonForSearch').on('click', function() {
  ...
  staName(departure, arrival, function(staArr) {
    for (i = 0; i < staArr.length; i++) {
      if (
        (departure === staArr[i].stationName ||
          departure == staArr[i].stationName.match(/^\S*/g)) &&
        staArr[i].passengerTraffic === true
      ) {
      ...
      if (
        (arrival === staArr[i].stationName ||
          arrival == staArr[i].stationName.match(/^\S*/g)) &&
        staArr[i].passengerTraffic === true
      ) {
        console.log(staArr[i].stationName);
        console.log(staArr[i].stationShortCode);
    ...
    junaData(depSta, destSta, function(nextTrainsArr) {
      ...
      for (i = 0; i < nextTrainsArr.length; i++) {
        ...
        for (j = 0; j < nextTrainsArr[i].timeTableRows.length; j++) {
          if (
            depSta === nextTrainsArr[i].timeTableRows[j].stationShortCode &&
            nextTrainsArr[i].timeTableRows[j].type === 'DEPARTURE'
          ) {
            ...
            var depDate = new Date(
              nextTrainsArr[i].timeTableRows[j].scheduledTime
            );

            var depDateFin = new Date(
              nextTrainsArr[i].departureDate
            ).toLocaleDateString('fi-FI', options);

            var depTime = new Date(
              nextTrainsArr[i].timeTableRows[j].scheduledTime
            ).toLocaleTimeString('fi', optiot);
          ...
          if (
            destSta === nextTrainsArr[i].timeTableRows[j].stationShortCode &&
            nextTrainsArr[i].timeTableRows[j].type === 'ARRIVAL'
          ) {
