...
$(function() {
  ...
  function sendSubmit(e) {
    ...
    const { id, username, password, login_type, result } = event.target;
    const handleUser =
      login_type.value == 'register' ? registerUser : loginUser;
    const reaction = handleUser(username.value, password.value);
    result.innerHTML = reaction;
  }
  function registerUser(username, password) {
    var details = [
      { id: username, details: { username: username, password: password } }
    ];
  ...
  function loginUser(username, password) {
    ...
    if (userCheck === null) {
      ...
      if (validUser && validPassword) {
        ...
        return `Väärä salasana käyttäjälle ${username}`;
      }
    }
  }
});
